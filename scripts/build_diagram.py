...
C = {"ui": "#DCEBFF", "api": "#E6F4EA", "graph": "#FFF8E1", "data": "#F3E8FF", "tools": "#FFE4E1", "obs": "#EEEEEE"}
W, H = 1.5, 0.7  # node size


def box(ax, x, y, w, h, text, color, fontsize=9, bold=False):
    ax.add_patch(mp.FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.02,rounding_size=0.12", fc=color, ec="#333", lw=1))
...
def arrow(ax, p1, p2, text="", style="-|>", ls="-", dx=0.0, dy=0.12):
    ax.annotate("", xy=p2, xytext=p1, arrowprops={"arrowstyle": style, "color": "#333", "lw": 1.1, "linestyle": ls, "shrinkA": 0, "shrinkB": 0})
    if text:
        ax.text((p1[0] + p2[0]) / 2 + dx, (p1[1] + p2[1]) / 2 + dy, text, fontsize=7.5, ha="center", color="#444")
...
def main() -> None:
    ...
    box(ax, 3.5, 7.5, 3.2, 1.8, "FastAPI (async)\nJWT auth + RBAC\ntoken-bucket rate limit\nSSE streaming", C["api"], bold=True)
    box(ax, 7.3, 7.5, 3.9, 1.8, "LangSmith\ntraces: every turn, node transition,\nLLM call, retrieval, tool call\n+ user feedback", C["obs"], bold=True)
    box(ax, 11.6, 7.5, 4.1, 1.8, "Security controls\ninjection screening (input + documents)\ninput / tool / content validation\noutput guard: citations, secrets, brand", C["obs"], fontsize=8.5, bold=True)
    ...
    # ---- graph container
    ax.add_patch(mp.FancyBboxPatch((0.3, 0.5), 10.8, 6.5, boxstyle="round,pad=0.02", fc=C["graph"], ec="#b8860b", lw=1.4))
    ax.text(0.5, 6.7, "LangGraph orchestration  (checkpointed per thread; every node is a LangSmith span)", fontsize=10, fontweight="bold")

    N = {  # name: (x, y)
        "guard": (0.6, 3.9), "memory_load": (2.35, 3.9), "supervisor": (4.1, 3.9),
        "retrieval": (6.2, 5.5), "research (RLM)": (6.2, 3.9), "tools": (6.2, 2.4), "approval\n(interrupt)": (6.2, 0.8),
        "response": (8.0, 3.9), "validator": (9.3, 2.4), "rewrite": (9.3, 0.8), "memory_update": (9.3, 5.5),
    }
    ...
    box(ax, 11.6, 5.8, 4.1, 1.1, "Pinecone (dense vectors)\nnamespace per department, metadata filters", C["data"])
    box(ax, 11.6, 4.5, 4.1, 1.1, "BM25 sparse index\nidentifier-aware tokens", C["data"])
    box(ax, 11.6, 3.2, 4.1, 1.1, "Memory: checkpointer (thread state)\n+ long-term user profile (JSON)", C["data"])
    box(ax, 11.6, 0.8, 4.1, 2.0, "Tool registry\nRBAC -> approval -> validation -> timeout\nknowledge_search | python_analysis (sandbox)\nmcp.* (directory, services, incidents)\nescalate_incident / reindex (admin + HITL)", C["tools"], fontsize=8)
    arrow(ax, (7.7, 6.2), (11.6, 6.45), "hybrid search: dense (Pinecone) + sparse (BM25) -> RRF -> rerank", dy=0.15)
