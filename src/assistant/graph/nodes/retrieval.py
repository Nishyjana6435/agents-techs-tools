...
def _fallback(state: AssistantState, exc: Exception) -> dict[str, Any]:
    return {
        "evidence": [],
        "retrieval_summary": f"retrieval unavailable: {exc.__class__.__name__}",
        "degraded": True,
    }
...
async def retrieval_node(state: AssistantState) -> dict[str, Any]:
    ...
    emit(
        "retrieval",
        "retrieval",
        f"hybrid search: '{state['question'][:80]}' filters={filters or 'none'}",
        params=params,
    )
    ...
    emit(
        "tool_result",
        "retrieval",
        f"knowledge_search -> {len(evidence)} evidence chunks",
    ...
    return {
        "evidence": evidence,
        "retrieval_summary": output["summary"] + (f"; notes: {'; '.join(notes)}" if notes else ""),
