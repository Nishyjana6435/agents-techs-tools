...
def _validator_fallback(state: AssistantState, exc: Exception) -> dict[str, Any]:
    return {"final_answer": state.get("draft_answer", ""), "validation": {"passed": False, "issues": [f"validator error: {exc}"]}}
...
async def validator_node(state: AssistantState) -> dict[str, Any]:
    ...
    validation = {"passed": report.passed, "issues": report.issues, "redactions": report.redactions, "invalid_citations": report.invalid_citations, "attempt": rewrite_count + 1}
    emit("validation", "validator", f"guardrails {'passed' if report.passed else 'FAILED'}: {report.summary()}", **validation)
    ...
    if rewrite_count >= MAX_REWRITES:
        ...
        emit("validation", "validator", "rewrite budget exhausted; delivering sanitised answer with disclaimer")
...
async def rewrite_node(state: AssistantState) -> dict[str, Any]:
    ...
    emit("validation", "rewrite", f"rewriting draft to fix: {'; '.join(issues)}", attempt=state.get("rewrite_count", 0) + 1)
    prompt = (
        f"Problems found:\n- " + "\n- ".join(issues) + "\n\n"
        f"Valid evidence ids: 1..{len(state.get('evidence') or [])}\n\n{render_evidence(state.get('evidence') or [], max_chars_each=500)}\n\n"
        f"DRAFT:\n{state.get('draft_answer', '')}"
    )
    msg = await llm_call(get_llm("primary"), [SystemMessage(content=REWRITE_SYSTEM), HumanMessage(content=prompt)])
